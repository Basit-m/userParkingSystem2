...
public class BookingTab extends JPanel{
    ...
    private void book(ActionEvent e) {
        ...
        try {
            ...
            for(Booking b : space.getTempBookings()) {
                if(b.getUser().equals(user) && b.getSpace().equals(space) && b.getStartTime().equals(startTime) &&
                   b.getEndTime().equals(endTime) && b.getLicensePlate().equals(license)) {
                    space.removeFromTemp(b);
                    user.addAmountOwed(user.getRate() * -1);
                }
            }
            space.addToTemp(booking);


            //space.addBooking(booking);
            status.setText("Booked for " + user.getName() + "! est. Cost: " + String.format("%.2f",booking.calculateFees()));
            user.addAmountOwed(user.getRate());
